...
class CryptoEngineService:
    ...
    def build_merkle_leaf(self, data: Any, index: int) -> MerkleNode:
        ...
        hash_value = self.sha256(str(index) + str(data))
    ...
    def build_merkle_tree(self, leaves: List[MerkleNode]) -> MerkleNode:
        ...
        padding = [MerkleNode(hash=self.sha256(""), is_leaf=True) 
        ...
        while len(current_level) > 1:
            ...
            for i in range(0, len(current_level), 2):
                ...
                parent_hash = self.sha256(left.hash + right.hash)
    ...
    def verify_merkle_proof(
        ...
        leaf_hash = self.sha256(str(proof.leaf_index) + str(leaf_data))
        ...
        for step in proof.proof:
            if step["direction"] == "left":
                ...
                current_hash = self.sha256(step["hash"] + current_hash)
            else:
                # Our hash is left, combine with right sibling
                current_hash = self.sha256(current_hash + step["hash"])
